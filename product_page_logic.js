// FINAL, CORRECTED product_page_logic.js: Renders the dynamic product page
...
(function() {
    ...
    // FIX: Correctly calculates and sets the top margin to push content below fixed headers.
    const positionWrapper = () => {
        if (!wrapper) return;
        let topOffset = 0;
        // It now ONLY looks for your specific nav bars and ignores all other fixed elements.
        document.querySelectorAll('.top-bar-nav, .ff7-nav-bar, #reduce-motion-container').forEach(el => {
    ...
    // --- CORE LOGIC (Including add to cart logic proxy) ---
    window.addSelectedVariantToCart_universal = function() {
        if (typeof window.addToCart !== 'function') return renderDiagnostic(`CART ERROR: Main cart system (addToCart) not loaded. Ensure your cart embed is placed first.`);
    ...
    async function initializePage() {
        ...
        try {
            ...
            if (Array.isArray(physicalProductsData)) {
                // If it's a simple array (old format), use it directly
                allPhysicalProducts = physicalProductsData;
            } else if (physicalProductsData && physicalProductsData.categories) {
                // Modular 3-level loading logic
                const categoryPromises = physicalProductsData.categories.map(category => fetch(`${category.url}?t=${new Date().getTime()}`).then(res => res.ok ? res.json() : Promise.reject(new Error(`Failed to fetch category file: ${category.name}`))));
                ...
                allPhysicalProducts = await Promise.all(productPromises);
            }
            
            allBaseProducts = [...digitalProducts, ...allPhysicalProducts];
    ...
    if (typeof window.hasInitializedProductPage === 'undefined') {
        ...
        // Final Fix: Run initialization immediately if the page is already loaded
...
// Call the function wrapper to define it globally
if (typeof initializeProductPage !== 'undefined') {
